...
function DFS(graph, root){
    ...
    const recursion = {}; // Memory of edges already travelled
    const path = []; // Paths taken through nodes from origin

    const exhausted = [root]; // List of nodes whose successors have been exhausted
    ...
    // Depth-first search
    function searchVariant(edge){ // Run search through queue and record path
        ...
        exhaustedSuccessors=0

        for (const successor of successors) { // Add each successor node to the depth-first search queue

            // Increment count of exhausted successors if they are exhausted
            if (successor in exhausted){
                exhaustedSuccessors ++;
            };
        ...
        // If all of a node's successors have been exhausted then exhaust this node
        if (exhaustedSuccessors == successors.length){
            
            // Record node as exhausted
            exhausted[edge.target]=true;
            
            // Queue - Remove current edge from the queue of edges to search
            queue.shift();

            console.log('exhausted node', edge.target);
        }
